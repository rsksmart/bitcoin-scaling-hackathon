...
contract Treasury is Lending {
    ...
    event receiveFundEvent(uint indexed amount);
    event withdrawEvent(address indexed borrower, uint indexed amount);

    function receiveFund() public payable {
        uint amt = super.calculateInterest();
        require(msg.value == amt, "Repay The correct Amount");
        super.resetValue();
        emit receiveFundEvent(msg.value);
    }

    function withdraw(uint _collateralVal) public payable {
